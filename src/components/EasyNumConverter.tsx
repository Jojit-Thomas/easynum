...
import { currencySymbols, currencyMap, formatNumberWithCommas, determineAutoFormat, convertNumberToReadableFormat, convertCurrency, mapCurrency } from "../utils/currency";
...
const EasyNumConverter: React.FC<EasyNumConverterProps> = ({ initialCurrency }) => {
  ...
  const formatNumber = async () => {
    ...
    const convertedFormat = format === "auto" ? determineAutoFormat(parseFloat(convertedAmount.amount), currency === "USD" ? "INR" : "USD") : format;
    const formattedConvertedAmount = convertNumberToReadableFormat(parseFloat(convertedAmount.amount), convertedFormat);

    setResult(num === 0 ? "Formatted result will appear here" : `${convertedAmount.symbol}${formatNumberWithCommas(formattedConvertedAmount, currency === "INR" ? "USD" : "INR")} (${currencySymbols[currency] || ""}${formatNumberWithCommas(formattedNumber, currency)})`);
  };
  ...
  const getDisplayedCurrencies = () => {
    if (showAllCurrencies) {
      ...
      return [detectedCurrency === "USD" ? ["GBP", "GBP"] : ["USD", "USD"], [detectedCurrency, detectedCurrency]].filter((item, index, self) => index === self.findIndex((t) => t[1] === item[1]));
    }
  };
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
        <div className="relative col-span-1">
          ...
          <input type="number" value={numberInput} onChange={(e) => setNumberInput(e.target.value)} placeholder="Enter a number" className="input w-full p-3 pl-10 border border-gray-300 rounded-md" />
        </div>
      ...
      <div className="flex items-center mb-4">
        <input id="showAllCurrencies" type="checkbox" className="mr-2 h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" checked={showAllCurrencies} onChange={(e) => setShowAllCurrencies(e.target.checked)} />
